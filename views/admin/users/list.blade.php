...
@section('content')
    <h1 class="page-header mb-6">@lang('subscriptions::admin/users.listTitle')</h1>
<div class="my-4 text-right">
    <a class="button small mr-4" href="/users/create">
        @lang('subscriptions::admin/users.create')
    </a>
</div>
...
<div class="text-center flex flex-col sm:flex-row justify-between mx-4">
    <div class="text-center">
        @if($list == 'all')
            <span class="selector active">@lang('subscriptions::admin/users.listAll') ({{ $stats['all'] }})</span>
        @else 
            <a class="selector" href="/users">@lang('subscriptions::admin/users.listAll') ({{ $stats['all'] }})</a>
        @endif

        @if($list == 'search')
            <span class="selector active">@lang('subscriptions::admin/users.listSearchResults') ({{ $stats['search'] }})</span>
        @endif
    </div>
    <div class="sm:ml-2">
        <form action="/users/search" role="search" style="white-space: nowrap;">
            <input class="py-1 px-3 w-auto border border-r-0 rounded-l-lg border-gray-400 focus:outline-none" 
                type="text" name="q" value="{{ $query ?? '' }}" placeholder="@lang('subscriptions::admin/users.search')" required><button type="submit" class="py-1 px-3 border rounded-r-lg border-indigo-500 bg-indigo-500 text-white">
...
@empty
    <div class="p-4 m-4 border rounded-lg border-gray-200 text-center text-gray-700">
        @lang('subscriptions::admin/users.listEmpty')
    </div>
@endforelse
